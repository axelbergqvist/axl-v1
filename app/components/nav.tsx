            ...
            {Object.entries(navItems).map(([path, { name }]) => {
              return (
                <Link
                  key={path}
                  href={path}
                  className={`transition-all duration-300 sm:text-xs text-sm ${isActive(path) ? 'text-white' : 'text-neutral-400 dark:text-neutral-400'} ${isActive(path) ? '' : 'hover:text-neutral-500 dark:hover:text-neutral-40'} align-middle`}
                >
                  {name}
                </Link>
              )
